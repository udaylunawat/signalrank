...
class LinkedInRapidAPIScraper:
    ...
    def search(
        ...
        *,
        ...
        limit = getattr(self.cfg, "max_results_per_query", 50)
        page_limit = getattr(self.cfg, "page_limit", 1)
    ...
    def _call_api(
        ...
        *,
        ...
        if self.logger and normalized:
            self.logger.info(
                f"[LINKEDIN_API] {endpoint} → {len(normalized)} jobs"
            )
    ...
    def _normalize_row(self, row: Dict) -> Optional[Dict]:
        ...
        elif isinstance(row.get("countries_derived"), list) and row["countries_derived"]:
