...
def _fast_role(title: str) -> str | None:
    ...
    if any(
        k in t for k in ["manager", "lead", "head", "director", "principal manager"]
    ):
        return "manager"

    if any(
        k in t
        for k in ["engineer", "developer", "scientist", "architect", "researcher"]
    ):
    ...
    for i in range(0, len(llm_needed), batch_size):
        ...
        try:
            ...
            for j, original_idx in enumerate(batch_idxs):
                ...
                # write cache
                _cache_key(titles[original_idx]).write_text(json.dumps({"role": role}))
